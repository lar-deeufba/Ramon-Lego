...
Mat mi = (Mat_<double>(3,1) << 0,0,0);         // Média do estado do filtro de Kalman
...
double Xdes = 1;      	double Ydes = 0.3;		double theta_des = pi/2;
double direcao;
// Janela do OpenCV
...
//	cout<<"\n "<<translation(0)<<"  "<<translation(1)<<"  "<<translation(2)<<endl;
    ...
    Mat PosicaoTag(const int& id){  
      Mat out;
        switch (id){
	    case 1:
		out = (Mat_<double>(1,3) << 0, 0, 1);
		break;
	    case 2:
		out = (Mat_<double>(1,3) << 0, -0.5, 2);
		break;
	    case 3:
		out = (Mat_<double>(1,3) << 0, 0.5, 3);
		break;
	    case 4:
		out = (Mat_<double>(1,3) << 0.75, 2, 4);
		break;
	    case 5:
		out = (Mat_<double>(1,3) << 0, 2, 5);
		break;
	    case 6:
		out = (Mat_<double>(1,3) << 0, 2, 6);
		break;
	    default:
    ...
    Mat CompatibilizaCoordenadas(const Mat& Dados, const int& id){    
	...
	double Rot, Rot_ant;
        ...
        Rot_ant = Dados.at<double>(0,2);
        DadosVelho = (Mat_<double>(3,1) << Dados.at<double>(0,0),
					   Dados.at<double>(0,1),
					             1           );
...
//             << endl;
    ...
    /* Processa imagem                                                             */
    Mat processImage(cv::Mat& I, cv::Mat& Ibw, const bool& FlagI) {
...
//		     "\n\tUd = " << Dados.at<double>(2,0) << "  Ue = " << Dados.at<double>(3,0) << std::endl;
	...
	if(FlagI == 1){
	   for(int i = 0; i < m/3; i++){
		if(i>0){
		    vconcat(Z, DadosSensoriais.row(i*3).t(), Z);
		    vconcat(PTag, DadosSensoriais.row(i*3 + 1).t(), PTag);
		    vconcat(PoseI, DadosSensoriais.row(i*3 + 2).t(), PoseI);
		}
		else{
		    Z = DadosSensoriais.row(i*3).t();
		    PTag = DadosSensoriais.row(i*3 + 1).t();
    		    PoseI = DadosSensoriais.row(i*3 + 2).t();
		}
	    }
	    // Calcula a média das estimações
	    for(int i=0; i<m/3; i++ ){
		    Xest = PoseI.at<double>(i*3,0) + Xest;
		    Yest = PoseI.at<double>(i*3 + 1,0) + Yest;
		    theta_est = PoseI.at<double>(i*3 + 2,0) + theta_est;
	    }
	    Xest = 3*Xest/m;
	    Yest = 3*Yest/m;
	    ...
	    direcao = Direcao(mi.at<double>(0,0) - Xdes,  mi.at<double>(1,0) - Ydes,
	     		                                        mi.at<double>(2,0));
	    // Descomente para imprimir na tela a postura inicial estimada
	    cout<<"\n Postura Inicial  "<<mi<<endl<<endl;
	}else{
	    for(int i = 0; i < m/2; i++){
		if(i>0){
	...
	// Descomente para mostrar os dados do sensor visual
//	cout<<"\n Z  "<<Z<<endl<<endl;
	// Cria arquivo com log para os dados do sensor visual
	double mZ = Z.rows;
	if(FlagI == 1){
	    for(int i=0; i<mZ/3; i++ ){
		Historico( Z.at<double>(i*3,0),Z.at<double>(i*3 + 1,0),
		      Z.at<double>(i*3 + 2,0),mZ/3,Xest,Yest,theta_est,0,0,0,pt_txt2);	
	    }
       	    FlagI = 0;
	}else{
	    for(int i=0; i<mZ/3; i++ ){
		Historico(Z.at<double>(i*3,0),Z.at<double>(i*3 + 1,0),
			            Z.at<double>(i*3 + 2,0),mZ/3,0,0,0,0,0,0,pt_txt2);	
	    }
	}
...
//	    Ek.at<double>(2,0) = theta_est;
	...
	while (true){    
	        // Captura frame
	        m_cap >> I;
	        DadosSensoriais = processImage(I, Ibw, FlagI);
