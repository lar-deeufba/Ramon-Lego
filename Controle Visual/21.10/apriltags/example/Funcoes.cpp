...
Mat Kalman(const Mat& Z, const Mat& PTag, const double& Wd, const double& We, 
    ...
    for(int i=0; i<m/3; i++){
	double deltai_x = PTag.at<double>(i*3,0) - x_est;
	double deltai_y = PTag.at<double>(i*3 + 1,0) - y_est;
	Mat deltai = (Mat_<double>(2,1) << deltai_x, deltai_y);
	cout<<"\n DELTAI "<<deltai<<endl<<endl;
	double q = norm(deltai,NORM_L2)*norm(deltai,NORM_L2);
