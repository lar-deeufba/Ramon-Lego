...
const double l = 0.07;      	     // Distância entre o centro do eixo e a roda (m)
...
/*  Para evitar repetições, duplicidades e uma boa inicialização,  é definida uma 
    funções para iniciar as constantes do controlador do model "polar".            */
Mat ConstPol(){ 
    // Cria matriz de parâmetros K_alfa, K_beta, e K_r opção 0.32, 0.113, ou 0.96, 0.42
    static const Mat Kk = (Mat_<double>(3,1) << 0.15, 0.002, 0.1);
    return Kk;
}
...
Mat Controle(const double& deltax, const double& deltay, const double& theta, 
    ...
    Mat Kdiffp = ConstPol();
    const double Kalfa = Kdiffp.at<double>(0,0);
    const double Kbeta = Kdiffp.at<double>(1,0);
    const double Kr = Kdiffp.at<double>(2,0); 
...
//    if(direcao == -1){
//	UVp = -V;
//	double delta = -standardRad((atan2(-deltay,-deltax)) - standardRad(theta)) - pi;
//	double Theta = -standardRad(-standardRad(theta) + delta + theta_d) - pi;
//   }
...
Mat Kalman(const double& X, const double& Y, const double& theta, 
    ...
    double err_x = 0.05;                                                       // (m)
    double err_y = 0.05;                                                       // (m)
    double err_theta = 0.09;                                          // 5.1566 graus
